...
public class GroupbyCode {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Long> result = list.stream().collect(Collectors.groupingBy(Function.identity(), Collectors
                .counting())
        );
        ...
        Map<BigDecimal, List<Item>> groupingByPrice = items.stream().collect(Collectors.groupingBy(Item::getPrice));
        System.out.println(groupingByPrice);

        Map<BigDecimal, Set<String>> groupingNames = items.stream().collect(Collectors.groupingBy(Item::getPrice,
